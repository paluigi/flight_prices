...
def get_flight_prices(
    ...
    try:
        # Individual flights
        flights = amadeus_client.shopping.flight_offers_search.get(originLocationCode=origin,
        ...
        for f in flight_lists:
            ...
            try:
                response_all_flights.extend(amadeus_client.shopping.flight_offers.pricing.post(f).data.get("flightOffers", []))
            except:
                continue
            # Price analysis
...
def is_first_or_third_wednesday():
    ...
    if today.weekday() == 2:  # Wednesday corresponds to weekday() value 2 (0 = Monday, 1 = Tuesday, ...)
