...
def get_flight_prices(
    origin: str, destination: str, advance_days: int, stay_days: int, limit: int = 20
) -> pd.DataFrame:
        ...
        for f in flight_lists:
            ...
            try:
                response_all_flights.extend(
                    amadeus.shopping.flight_offers.pricing.post(f).data.get(
                        "flightOffers", []
                    )
                )
            except:
...
data_df.to_csv(
    "flight_prices_{}.csv".format(datetime.now().strftime("%Y-%m-%d")), index=False
)
